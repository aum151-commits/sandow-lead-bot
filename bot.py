...
def on_message(msg):
    ...
    if awaiting and text and not text.startswith("/") \
        ...
        # Заставить нажать Телеграм не позволяет, поэтому шаг подаём как
        # обязательный, без альтернатив, и один раз напоминаем через 45 минут.
        if MANAGER_TG_URL:
            api("sendMessage", chat_id=chat_id,
                text=(f"Приятно познакомиться, {name}! 🤝\n\n"
                      "Остался один шаг: нажмите кнопку и отправьте нам любое "
                      "сообщение, хоть «привет». Без этого Телеграм не даст "
                      "нам написать вам первыми — так у него устроена защита "
                      "от спама."),
                reply_markup=kb_mixed([[("✍️ Открыть чат клуба",
                                         "url:" + MANAGER_TG_URL)]]))

            def _bridge_nudge(cid=chat_id):
                api("sendMessage", chat_id=cid,
                    text=("Напомним про последний шаг 🙂 Чтобы мы могли "
                          "отвечать вам в Телеграме, нажмите кнопку и напишите "
                          "нам что-нибудь. Если уже написали — пропустите это "
                          "сообщение."),
                    reply_markup=kb_mixed([[("✍️ Открыть чат клуба",
                                             "url:" + MANAGER_TG_URL)]]))
...
VERSION = "2026-08-15-v4-bridge-mandatory"
