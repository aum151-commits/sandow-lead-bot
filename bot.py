...
_MANAGERS = ["Рыбалко", "Пучко", "Плотникова", "Тоганидзе", "Спиридонова"]
_MANAGERS += [m.strip() for m in os.environ.get("MANAGERS", "").split(",") if m.strip()]

# Как менеджеры подписываются в чате: Лиза вместо Рыбалко и так далее.
_MANAGER_ALIASES = {
    "лиза": "Рыбалко", "елизавета": "Рыбалко",
    "вика": "Пучко", "виктория": "Пучко",
    "алина": "Плотникова",
    "мила": "Тоганидзе", "милана": "Тоганидзе", "милания": "Тоганидзе",
    "маша": "Спиридонова", "мария": "Спиридонова",
}

_STAFF_CACHE = {"когда": 0.0, "менеджеры": None, "псевдонимы": None, "тренеры": None}
...
def _refresh_staff():
    ...
    try:
        ...
        менеджеры, псевдонимы = [], {}
        for m in данные.get("менеджеры", []):
            фамилия = m.get("фамилия")
            if not фамилия:
                continue
            менеджеры.append(фамилия)
            for зовут in [m.get("имя")] + list(m.get("зовут") or []):
                if зовут:
                    псевдонимы[зовут.lower()] = фамилия
        if менеджеры:
            _STAFF_CACHE["менеджеры"] = менеджеры
            _STAFF_CACHE["псевдонимы"] = псевдонимы
...
def _manager(text):
    ...
    псевдонимы = _STAFF_CACHE["псевдонимы"] or _MANAGER_ALIASES

    низкий = text.lower()
    for фамилия in фамилии:
        if фамилия.lower()[:-1] in низкий:
            return фамилия
    for зовут, фамилия in псевдонимы.items():
